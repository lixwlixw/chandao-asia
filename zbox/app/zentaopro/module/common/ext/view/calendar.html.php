...
function adjustWidth()
{
    var weekendEvents = 0;
    var width = 80;
    $('#calendar tbody.month-days .event').each(function()
    {
        var day   = new Date($(this).closest('.day').attr('data-date'));
        var week  = day.getDay();
        var days  = $(this).attr('data-days');
        var style = $(this).attr('style');
        style     = style.substr(style.indexOf('width'));
        dayWidth  = style.substr(0, style.indexOf(';') - 1).substr(7, 1);
        if(days != dayWidth && week == 1) $(this).css('width', (dayWidth - 1) * 100 + '%');
        if(week == 0 || week == 6)
        {
            weekendEvents += 1;
        }
        else if(days > 1 && (week + days - 1) > 6)
        {
            weekendEvents += 1;
        }
    });
